...
#$win = PDL::Graphics::PGPLOT::Window->new(Device=>"$ENV{HOME}/Software/Figures/$id-SED_VIzK1234.ps/cps"); 
    $win = PDL::Graphics::PGPLOT::Window->new(Device=>"$ENV{HOME}/Software/IRexcess/Figures/cce/$id-SEDcombined.ps/vcps"); 
#    $win = PDL::Graphics::PGPLOT::Window->new(Device=>'/xs');
    ...
    $win->points(log10($wav0($ix)), $plotlogflux,{Col=>Red,SymbolSize=>3});
...
#plot stellar comp seperately
    $win->line(log10($wzst), log10( $szst * ($wzst/10000)**2 / 3e-16 ),{LineStyle=>'dashed',Color=>'red'}); # Plotting in Jy
#    $win->points(log10($wav0($ix)),log10($flux($ix)),{Col=>blue}); #plotting model as mags
    ...
    $win->text("$id: z = $zsp",4.2,max($plotlogflux)-1.9,{Charsize=>1.2});
    $win->text("SpType = $spectralclass",log10(3400),max($plotlogflux)+0.3);
#    $win->close();
    
#now we can do it for PEGASE fit without cce component to VIzK12 mags
    
    open IN3, "$ENV{HOME}/Software/Masses/GDDSVIzK12/SEDparameters.dat";
...
#    $win->line(log10($wz), log10( $sz * ($wz/10000)**2 / 3e-16 ),{LineStyle=>'Dashed',Color=>'purple'});
#    $win->legend(["VIz'K + IRAC + Starburst SED fit","VIz'K + IRAC SED fit"],log10(10000),max($plotlogflux)-2.3,{LineStyle=>['Solid','Dashed'],TextFraction=>0.75});
	...
	if (-e "$ENV{HOME}/Software/IRexcess/fits_images/".$id."_F814W.fits") {
	    chdir "$ENV{HOME}/Software/paper/figures/images";
	    $filejpg = join('',$id,'_3_6.jpg');
	    $fileps = join('',$id,'_3_6.ps');
	    system "convert $filejpg $fileps";

	    $filejpg = join('',$id,'_4_5.jpg');
	    $fileps = join('',$id,'_4_5.ps');
	    system "convert $filejpg $fileps";

	    $filejpg = join('',$id,'_5_8.jpg');
	    $fileps = join('',$id,'_5_8.ps');
	    system "convert $filejpg $fileps";

	    $filejpg = join('',$id,'_8_0.jpg');
	    $fileps = join('',$id,'_8_0.ps');
	    system "convert $filejpg $fileps";

	    $filejpg = join('',$id,'_F814W.jpg');
	    $fileps = join('',$id,'_F814W.ps');
	    system "convert $filejpg $fileps";

	    $filejpg = join('',$id,'_24.jpg');
	    $fileps = join('',$id,'_24.ps');
	    system "convert $filejpg $fileps";


           # $winfits = PDL::Graphics::PGPLOT::Window->new(Device=>"$ENV{HOME}/Software/IRexcess/Figures/$id-fits.ps/cps"); 
