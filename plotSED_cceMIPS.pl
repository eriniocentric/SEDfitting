...
#@id_list = ("SA22-0590","SA22-2639","SA22-1813","SA22-2172","SA22-0448","SA22-1055","SA22-0281","SA22-0630");
#$mipsflux = pdl(73.51,374.94,72.04,269.87,285.47,383.67,203.59,140.46); #in muJY
#$mipsflux /= 1e6; #in Jy to match plots
$wavmips = 236800.; 
...
$i = 0;
$nondetection = 0; #count non-detections
$detection = 0;
open OUT, ">$ENV{HOME}/Software/MIPS/compareMIPSmodels.txt";

open MIPS, "$ENV{HOME}/Software/Catalogs/MIPSfluxcombine.txt";

while (<MIPS>) {
  @vmips = split;
  $id = $vmips[0]; $mipsflux = $vmips[1] * 1e-6; #convert MIPS flux to Jy
...
#  foreach $id (@id_list) {
  ...
  while(<IN>) {
    ...
    next if $Conf <=1 or $Conf > 7 ; # Good photo-z only (and not star)
    $z = $zsp;
...
$dflux_v(4:7) *= 2; #multiply spitzer errors by 2
...
#get model flux through MIPS filter curve

$mipsmodel = flux($wz,$sz * ($wz/10000)**2 / 3e-16 ,'mips');
$mipsmodel = $mipsmodel->at(0);
# $sz now matches that used to produce the colors for this model

# redshift burst component for plot
...
#$win = PDL::Graphics::PGPLOT::Window->new(Device=>"$ENV{HOME}/Software/Figures/$id-SED_VIzK1234.ps/cps"); 
    $win = PDL::Graphics::PGPLOT::Window->new(Device=>"$ENV{HOME}/Software/paper/figures/cce/MIPS/$id-SEDcombined.ps/vcps"); 
#    $win = PDL::Graphics::PGPLOT::Window->new(Device=>'/xs');
    $plotlogflux = log10($dflux($ix));
    $dplotlogflux = 0.4343 * sqrt($dflux_v($ix)) / $dflux($ix);
if ($mipsflux < 70e-6) { #all fluxes less than 70 mJy plot as upper limits
  $logmipsflux = log10(70e-6);
}
else {
  $logmipsflux = log10($mipsflux);
}
 
    $win->env(log10(3000),log10(300000),max($plotlogflux)-2.5,$logmipsflux+0.7,{Axis=>'LogXY',XTitle=>'\gl (\A)',YTitle=>'F\d\gn\u (Jy)'});
    $win->points(log10($wav0($ix)), $plotlogflux,{Col=>Red,SymbolSize=>4});
...
if ($mipsflux < 70e-6) { #all fluxes less than 70 mJy plot as upper limits
  $nondetection += 1;
  $logmipsflux = log10(70e-6);
  $win->points(log10($wavmips),$logmipsflux,{Col=>purple,Symbol=>31,SymbolSize=>3}); 
}
else {
$detection += 1;
  $logmipsflux = log10($mipsflux);
  $win->points(log10($wavmips),$logmipsflux,{Col=>purple,SymbolSize=>4,Symbol=>16}); 
  $win->errb(log10($wavmips),$logmipsflux,0.2,{Col=>purple});
}
 
    $obs33 = ones(2) * 3 * (1+$z) * 1e4;
    #for reference plot 3 microns
    $win->line(log10($obs33),pdl(-1,-10),{Color=>red,Linestyle=>solid});
    $win->text('\gl\drest\u=3\gmm',log10($obs33((0)))+0.05,max($plotlogflux)-2.3,{Colour=>red,Charsize=>1.2});

#   $win->text('log\d10\uM = '.$massstarburst.'\(2233)'.$massstarbursterr." Stellar + Hot Dust", 4.1,max($plotlogflux)-2.3,{Charsize=>1.2});
#    $win->text('log\d10\uM = '.$massVIzK12.'\(2233)'.$massVIzK12err.' Stellar only', 4.1,max($plotlogflux)-2.1,{Charsize=>1.2});
#    $win->text("$id: z = $z",4.2,max($plotlogflux)-1.9,{Charsize=>1.2});
#    $win->text("SpType = $spectralclass",log10(3400),$logmipsflux+0.3);
   $win->text("$id: z = $zsp",log10(3400),$logmipsflux+0.05,{Charsize=>1.4});
    $win->text('F\d3\gmm\u(GB/PAH)/F\d3\gmm\u(stellar)='.sprintf( "%4.2f",$burstratio_avg)."+/-".sprintf( "%4.2f",$burstratio_err),log10(3400),$logmipsflux+0.35,{Charsize=>1.3});
    $win->text("SpType = $spectralclass",log10(3400),$logmipsflux-0.25,{Charsize=>1.4});
 
#print model MIPS and observed MIPS to new file
$id =~ s/SA22-//;
printf OUT "%10s %8.6g %8.6g \n",$id,$mipsflux,$mipsmodel;
