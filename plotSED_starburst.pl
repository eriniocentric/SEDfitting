...
#	$massVIzK = $v[43]; $massVIzKerr = $v[44];$massVIzK1234 = $v[60]; $massVIzK1234err = $v[61];
 #get masses directly from output file instead     
	...
	close FILE1;
	open FILE2, "$ENV{HOME}/Software/Masses/GDDSVIzK1234_starburst-masses_1kpc.dat" or die "Died on FILE2";
	while(<FILE2>) {
	    next unless m/$id/;
	    @v2 = split;
	    print @v2;
   ...
   $ix = which($dmags_v<1);  # This keeps all VIzK points, and throws out only a couple of really bad spitzer points
    ...
    open IN2, "$ENV{HOME}/Software/Masses/GDDSVIzK1234_starburst_1kpc/SEDparameters.dat";
...
# Redden the spectrum in the same way expand_massgrid2.p does
    ...
    $fileburst = "$ENV{HOME}/Software/starburst/SBgrid/1kpc/L1280Av035_1d4.plot";
...
#$win = PDL::Graphics::PGPLOT::Window->new(Device=>"$ENV{HOME}/Software/Figures/$id-SED_VIzK1234.ps/cps"); 
    $win = PDL::Graphics::PGPLOT::Window->new(Device=>"$ENV{HOME}/Software/IRexcess/Figures/$id-SEDcombined_SB.ps/cps"); 
#    $win = PDL::Graphics::PGPLOT::Window->new(Device=>'/xs');
    ...
    $win->text('log\d10\uM = '.$massstarburst.'\(2233)'.$massstarbursterr." IRAC + Starburst", log10(3200),max($plotlogflux)+0.5);
    $win->text('log\d10\uM = '.$massVIzK1234.'\(2233)'.$massVIzK1234err.' IRAC', log10(3200),max($plotlogflux)+0.38);
#    $win->close();
    
#now we can do it for PEGASE fit without Burst component only
...
# $sz now matches that used to produce the colors for this model
    ...
    $win->legend(["VIz'K + IRAC + Starburst SED fit","VIz'K + IRAC SED fit"],log10(28000),max($plotlogflux)-2.3,{LineStyle=>['Solid','Dashed'],TextFraction=>0.75});
    $win->close();
