...
#$win = PDL::Graphics::PGPLOT::Window->new(Device=>"$ENV{HOME}/Software/Figures/$id-SED_VIzK1234.ps/cps"); 
    $win = PDL::Graphics::PGPLOT::Window->new(Device=>"$ENV{HOME}/Software/Figures/sed/$id-SEDcombined.ps/vcps");
...
#    $win->points(log10($wav0($ix)),log10($flux($ix)),{Col=>blue}); #plotting model as mags
  ...
  #  $win->text('log\d10\uM = '.$massVIzK12.'\(2233)'.$massVIzK12err.' Stellar only', 4.1,max($plotlogflux)-2.1,{Charsize=>1.2});
    ...
    $win->text('log\d10\uM (VIzK+IRAC) = '.$massstarburst.'\(2233)'.$massstarbursterr.' ', log10(3200),max($plotlogflux)+0.5);
    $win->text('log\d10\uM (VIzK) = '.$massVIzK.'\(2233)'.$massVIzKerr.' ', log10(3200),max($plotlogflux)+0.38);
...
#open VIzK and plot best fit SED
