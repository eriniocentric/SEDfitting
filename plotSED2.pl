...
#first will do it for VIzK1234 mags
	...
	s/\*//;
	@v = split;
	$id1 = $v[0]; $z = $v[1]; $zph = $v[2]; $Conf = $v[5];
	$massVIzK = $v[57]; $massVIzKerr = $v[58];$massVIzK123 = $v[60]; 
	$massVIzK123err = $v[61]; $massVIzK1234 = $v[63]; $massVIzK1234err = $v[64];
   ...
   $ix = which($dmags_v<1);  # This keeps all VIzK points, and throws out only a couple of really bad spitzer points
    ...
    open IN2, "$ENV{HOME}/Software/Masses/Masses_GDDS8/SEDparameters.dat";
...
#$win = PDL::Graphics::PGPLOT::Window->new(Device=>"$ENV{HOME}/Software/Figures/$id-SED_VIzK123.ps/cps"); 
    $win = PDL::Graphics::PGPLOT::Window->new(Device=>"$ENV{HOME}/Software/Figures/$id-SEDcombined2.ps/cps"); 
#    $win = PDL::Graphics::PGPLOT::Window->new(Device=>'/xs');
    $plotlogflux = log10($dflux($ix));
    $dplotlogflux = 0.4343 * sqrt($dflux_v($ix)) / $dflux($ix);
    $win->env(log10(3000),log10(100000),max($plotlogflux)-2.5,max($plotlogflux)+0.9,{Axis=>'LogXY',XTitle=>'Wavelength (\A)',YTitle=>'Flux (Jy)',Title=>"$id: z = $zsp"});
    $win->points(log10($wav0($ix)), $plotlogflux,{Col=>Red,SymbolSize=>1.5});
    $win->errb(log10($wav0($ix)), $plotlogflux, $dplotlogflux,{Col=>Red});
    $win->line(log10($wz), log10( $sz * ($wz/10000)**2 / 3e-16 ),{Linestyle=>'Dashed'}); # Plotting in Jy
    $win->text('log\d10\uM = '.$massVIzK1234.'\(2233)'.$massVIzK1234err." With IRAC Ch 8", log10(3200),max($plotlogflux)+0.77);
    $win->text('log\d10\uM = '.$massVIzK123.'\(2233)'.$massVIzK123err.' Without IRAC Ch 8', log10(3200),max($plotlogflux)+0.65);
#    $win->close();
    
#now we can do it for VIzK123 mags only
    
    $ix = sequence(7);
    
    open IN3, "$ENV{HOME}/Software/Masses/Masses_GDDS9/SEDparameters.dat";
...
# $sz now matches that used to produce the colors for this model
    ...
    $win->line(log10($wz), log10( $sz * ($wz/10000)**2 / 3e-16 ),{LineStyle=>'Solid'});
    $win->legend(["With IRAC-ch8","Without IRAC-ch8"],log10(28000),max($plotlogflux)-2.3,{LineStyle=>['Dashed','Solid'],TextFraction=>0.75});
    $win->close();
}
